 ...
 * <br>
 * <br>
 * Design test: in this class every method tests a different method of
 * ListAdapter.
 * In every method the execution variables changes before calling the tested
 * method in order to test all the possible conditions
 * <br>
 * <br>
 * Description: The ListAdapter class implements HList and HCollection,
 * therefore the HCollection methods were tested using an HCollection as object
 ...
 * Preconditions:
 * <br>
 * A new empty object of type ListAdapter must always be instantiated before
 * each test
 * <br>
 * All execution variables are never in an uninitialized state (i.e. list =
 * null) unless explicitly desired
 * <br>
 ...
 * Execution variables:
 * <br>
 * HList list - main empty list on which all methods common to HCollection and
 * HList are tested
 * <br>
 * HList listWithData - non-empty list on which the iterator is tested
 * <br>
 * HCollection coll - object of type CollectionAdapter used to test methods that
 * accept parameters like HCollection and to test methods of HCollection
 * <br>
 * <br>
 *
...
public class ListTest {
     ...
     * <br>
     * <br>
     * Summary: test to verify the correct creation of an object of type
     ...
     * <br>
     * <br>
     * Summary: Method test that returns the current size of the Collection
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: the add () and remove () methods must be properly implemented
     * and working
     * <br>
     * <br>
     ...
     * The size of the collection must vary consistently as the insertion / removal
     * elements vary
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: The method that indicates whether the collection is empty is tested
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Postconditions: the method must return true if the container is empty and
     * therefore does not contain any elements
     * <br>
     * <br>
     * Expected results: false after insert, true after removal
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: The test verifies that the method returns true if the container
     ...
     * <br>
     * <br>
     * Preconditions: the add (Object) method is correctly implemented
     * <br>
     * <br>
     * Postconditions: the method must return true if the element passed is actually
     * contained, false otherwise
     * <br>
     * <br>
     * Expected results: true with manually entered data, false with data not
     * present
     */
    @Test
     ...
     * array is created with multiple calls of next () and elements with multiple
     * calls of remove are eliminated. Exceptions for next () on empty collection or
     * remove () are tested before calling next ()
     * <br>
     * <br>
     * Description: After creating the iterator, elements are added to the
     * collection with the add (Object) method. Subsequently an array of Object is
     * created, through a while loop that uses the hasNext () method, containing all
     * the elements of the collection and is compared with the array obtained
     * through the toArray () method. Finally, all the elements contained in the
     * collection are removed by invoking cyclically (with the same while
     * precedenet) the method next () and remove () to then verify that the
     * collection has size 0
     * <br>
     * <br>
     * Preconditions: the size () and toArray () and add () methods must be
     * correctly implemented and working
     * <br>
     * <br>
     * Postconditions: the iterator must be able to sequentially return and delete
     * each file contained in the Collection. It must also communicate the presence
     * or absence of other elements not removed
     * <br>
     * <br>
     * Expected results: the Object array is filled correctly with all the elements
     * of the collection obtained with the iterator and the final size of the
     * collection = 0
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: the previous test is performed with empty collection
     * <br>
     * <br>
     * Description: All previous methods are invoked the way they were invoked in
     * {@link #testIteratorMethods()} without adding elements to the collecton
     * <br>
     * <br>
     * Preconditions:
     * <br>
     * The size () and toArray () methods must be correctly implemented and working
     * <br>
     * Cannot be called remove () before calling next ()
     * <br>
     * Cannot be called next () with empty Collection otherwise throw exception
     * <br>
     * <br>
     * Postconditions: no exceptions thrown
     * <br>
     * <br>
     * Expected results: no exception is thrown and no segmentation fault appears
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: the test verifies that the toArray () method correctly returns an
     * array containing all the objects present in the collection in the order in
     ...
     * <br>
     * <br>
     * Preconditions: the collection can be either empty or partially filled
     * <br>
     * <br>
     * Postconditions: the method must return an array of Objects containing all the
     * elements contained in the collection, in the order in which they were
     * originally inserted
     * <br>
     * <br>
     * Expected results: the array obtained by the method and the one created
     * manually must match
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: comparable test with the toArray () method with the difference that,
     * if it is large enough, the array passed as a parameter is used
     * <br>
     * <br>
     * Design Test:
     * <br>
     * Nothing is inserted into the collection and a partially occupied array is
     * passed as a parameter
     * <br>
     * Some elements are inserted into the collection and a partially occupied array
     * with a larger size is passed as a parameter
     * <br>
     * Some elements are inserted into the collection and a partially occupied array
     * with a smaller size is passed as a parameter. Also test of the correct throw
     * of the exception with null parameter
     * <br>
     * <br>
     * Description: for all three situations indicated by the design test, it is
     * verified with the use of a manually created array that the tested method
     * returns an array of Object, filled with null the greater the difference
     * between the size of the array passed as parameter and the size of the
     * collection, exactly the elements of the collection, or a new array always
     * with the elements of the collection
     * <br>
     * <br>
     * Preconditions: The add (Object) method must be correctly implemented
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Expected results: the method must return the array passed as a parameter with
     * the objects of the collection, if large enough, otherwise return another one
     * with a size equal to the number of objects in the collection. If collection
     * empty, the array supplied as a parameter is not modified
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: the correct functioning of the add (Object obj) method is tested,
     * which adds an element ("in the queue" if considered the array returned by
     * toArray ()) to the collection
     * <br>
     * <br>
     ...
     * Preconditions:
     * <br>
     * The collection can be either empty or full
     * <br>
     * The toArray () method must be properly implemented and working
     * <br>
     * <br>
     * Postconditions: the collection must contain the value passed as a parameter
     * <br>
     * <br>
     * Expected results: Elements must be arranged in the array in the same way the
     * tested method is invoked
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: Verifies that the tested method removes the specified element or
     * returns false if it is not present
     * <br>
     * <br>
     * Design test:
     * <br>
     * An element from the empty collection is deleted
     * <br>
     * An element not present from a non-empty collection is deleted
     * <br>
     * An element present from a non-empty collection is deleted
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: the add (Object obj) method and the toArray () method must be
     * correctly implemented
     * <br>
     * <br>
     * Postconditions: after invoking the method, the collection must not contain
     * the data entered as a parameter, returning true, if the collection is not
     * modified it returns false
     * <br>
     * <br>
     * Expected results: after the removal of the specified element, the size of the
     * collection must be decreased by one unit, the first element corresponding to
     * the entered parameter must have been removed and true must be returned, with
     * data not present, however, false
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: the test verifies that the main collection contains all the elements
     * of the collection passed as a parameter
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: The add (Object obj) method must be correctly implemented
     * <br>
     * <br>
     * Postconditions: the method must return true if the collection contains all
     * the elements present in the collection passed as a parameter
     * <br>
     * <br>
     * Expected results: Only if the test collection contains only a subset of the
     * elements contained in the main collection then true is returned
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: the test verifies that the addAll (HCollection) method inserts all
     * the elements contained in the collection passed as a parameter in the
     * collection in which the method is invoked
     * <br>
     * <br>
     * Design test: initially the tested method is invoked with a null collection to
     * verify the correct throwing of the exception. Then a non-null collection is
     ...
     * <br>
     * <br>
     * Preconditions: the add (Object obj) method and the containsAll (HCollection
     * coll) method must be correctly implemented
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Expected results: after the invocation of the method, the collection also
     * contains the elements of the collection passed as a parameter
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: the method that removes all the elements contained in the collection
     * passed as parameter from the main collection is tested
     * <br>
     * <br>
     * Design test:
     * <br>
     * Check that if an empty collection is passed, the main collection is not
     * modified (neither if empty nor if not empty)
     * <br>
     * Check that if a collection with elements not contained in the main one is
     * passed, this does not change
     * <br>
     * Verify that if a collection with elements in common is passed, they are
     * deleted from the main collection
     * <br>
     * <br>
     ...
     * Preconditions:
     * <br>
     * The collection passed as a parameter must not be null
     * <br>
     * The add (Object obj) method and the toArray () method must be correctly
     * implemented
     * <br>
     * <br>
     * Postconditions: in the main collection there must be no elements contained in
     * the collection passed as a parameter
     * <br>
     * <br>
     * Expected results: false if an empty collection is passed, true if at least
     ...
     * <br>
     * <br>
     * Summary: the method that removes from the main collection all elements not in
     * common with the collection passed as a parameter is tested
     * <br>
     * <br>
     * Design test:
     * <br>
     * Main and parameter collection both empty, expected value false (no change)
     * <br>
     * Empty parameter collection, non-empty main collection, expected value true
     * (change, empty main collection)
     * <br>
     * Collection of parameter with values different from those of the main
     * collection, same result pt.2
     * <br>
     * A common value, expected value true, main collection dimension 1 with that
     * value
     * <br>
     * A common value and parent collection that single repeating value, expected
     * value true, and parent collection empty
     * <br>
     * <br>
     ...
     * Preconditions:
     * <br>
     * The collection passed as a parameter cannot be null, but it can be empty or
     * with repeated values
     * <br>
     * The add (Object obj) method, the toArray () and size () method must be
     * correctly implemented
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Design test: non-empty collection is created and is used as a parameter for a
     * non-empty list Throwing of exceptions in case of wrong index or null
     * collection is also tested
     * <br>
     * <br>
     * Description: after creating a non-empty collection and inserting the data
     * into the list using the add (Object obj) method defined in the
     * ListAdapter.add (Object) class, the collection is inserted in the three
     * points: initial, intermediate and final and the results are compared with a
     * manually created array with the expected values
     * <br>
     * <br>
     * Preconditions: the add (Object) method must be correctly implemented, as well
     * as the toArray () method, both of HCollection
     * <br>
     * <br>
     * Postconditions: the size of the list must be larger after the method is run
     * <br>
     * <br>
     * Expected results: the values of the collection must appear in the list at the
     * specified point
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: the equals method must return true s if the length of the two
     * compared lists is equal and the same elements are contained in the same order
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: the add (Object) methods of HList and HCollection must be
     * correctly implemented
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Summary: Text of the hash calculation method defined in HList
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: the hash calculation must be the one described in HList
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Summary: test to verify the correct return parameter of the get function
     * <br>
     * <br>
     * Design test: after inserting some elements, check that each inserted element
     * is reachable through the tested method
     * <br>
     * <br>
     * Description: after the manual insertion of values, a for loop is called to
     * verify that indeed each inserted element is correctly returned by the get
     * method
     * <br>
     * <br>
     * Preconditions: the index inserted must not be less than 0 or greater than the
     * number of elements inserted
     * <br>
     * <br>
     * Postconditions: the element returned must be the one contained in the i-th
     ...
     * <br>
     * <br>
     * Summary: The method must correctly set all elements of the list
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: The implementation of the add (Object), size () and equals
     * (Object) method of ListAdapter is working correctly
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Summary: Testing the method for adding an item to a specific location
     * <br>
     * <br>
     * Design test: given a for loop that inserts increasing numbers in the half of
     * the list containing the elements, the resulting list is compared to an array
     * built ad hoc containing the expected elements. Also verified the correct
     * throwing of the exceptions
     * <br>
     * <br>
     * Description: The method verifies that the call to the add (Integer, Object)
     * function inserts the specified element at the specified location. This is
     * verified at each insertion by comparing the array returned by the toArray ()
     * method with an array built ad hoc containing the expected values. Wrong index
     * entries are also performed to verify that exceptions are thrown correctly. It
     * is also verified that by inserting an element at the beginning of an empty
     * list it will become the first element of the list
     * <br>
     * <br>
     * Preconditions:
     * <br>
     * The index must be between 0 and the maximum size of the list
     * <br>
     * ListAdapter's toArray () method must be properly implemented and working
     * <br>
     * Object's getClass () method must be working correctly
     * <br>
     * <br>
     * Postconditions: elements must be added in the indicated position provided
     * that this is valid
     * <br>
     * <br>
     * Expected results: the returned arrays correspond to the elements inserted in
     * the position in which they were inserted
     */
    @Test
    public void testAdd() {
     ...
     * <br>
     * <br>
     * Summary: the remove method is tested which must correctly delete the element
     * contained in the specified position, resulting in a list with a size smaller
     * than 1 compared to before the invocation of the method
     * <br>
     * <br>
     * Design test: after adding values to the list, removing some of them must
     * result in a list of less than and equal size to a manually created checklist
     * <br>
     * <br>
     * Description: the list is filled with ints, for simplicity; a for loop is then
     * called to check if the element contained in the i-th position is to be
     * deleted (in this case if it is even) and if so it is removed. The resulting
     * list must correspond to a list created ad hoc containing the expected values.
     * The re-entry of an element into an empty list is also tested
     * <br>
     * <br>
     * Preconditions: ListAdapter's add (Object), get (int) and equals (Object)
     * methods must be properly implemented and working
     * <br>
     * <br>
     * Postconditions: the removed element must match the element contained in the
     * position indicated by the index passed as a parameter
     * <br>
     * <br>
     * Expected results: at the end of the test the list containing the additional
     * values is the same as the list with the expected values
     */
    @Test
     ...
     * <br>
     * <br>
     * Summary: Test for the method that looks for the position of the first element
     * corresponding to the element passed as a parameter
     * <br>
     * <br>
     * Design test: after inserting some elements I perform three searches for
     * single element, duplicate element, non-existent element. Search also with
     * empty list
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: the list object must not be null (but can be empty)
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Summary: Test for the method that looks for the position of the last element
     * corresponding to the element passed as a parameter
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Summary: Tests that the listIterator (int) method correctly returns an
     * iterator initialized at the point indicated by the parameter
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: the next () method of the list iterator and the size () method
     * of the list must be correctly implemented
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Summary: the method tests the correct creation of a sublist starting from the
     * original list
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Preconditions: the size (), toArray () and getClass () method must be
     * correctly implemented
     * <br>
     * <br>
     ...
     * <br>
     * <br>
     * Summary: This method tests the correct operation of all methods of the list
     * iterator
     * <br>
     * <br>
     * Design test: the entire scan is carried out first in one direction and then
     * in the other of the entire list, with the iterator, and the data are checked
     * manually, then a data is entered and the correct insertion is verified and
     * then removed by checking it correct removal. Finally, a datum present in the
     * list is modified and the change is verified by comparing the list to an array
     * created manually with the expected elements
     * <br>
     * <br>
     * Description: first of all the methods that do not throw exceptions are tested
     * using an iterator on an empty list, in this case the add (Object obj) method
     * and the previous () method which must return the value just inserted.
     * Subsequently, an array is created with a size equal to the length of the
     * list, and is filled by scanning the entire list with the iterator in both
     * directions, this array is compared with an array built ad hoc containing the
     * expected values. At the end of these two operations it is verified with
     * prevIndex () and nextIndex () that the iterator has actually reached the
     * start / end Finally, the add, remove and set methods are tested, which
     * insert, remove and modify data always in the same position of the list after
     * calling next () and previous () each.
     * <br>
     * <br>
     * Preconditions:
     * <br>
     * The toArray () and size () methods must be correctly implemented
     * <br>
     * The add (Object obj) and remove () methods must not be invoked if the next ()
     * or previous () methods have not been invoked first and they must not be
     * invoked twice in a row
     * <br>
     * The iterator is created on a non-empty list
     * <br>
     * <br>
     * Postconditions: the list iterator must allow you to navigate back and forth
     * along the entire list sequentially and to be able to modify, remove and add
     * data
     * <br>
     * <br>
     * Expected results: the list contains exactly the elements contained by the
     * manually created Object arrays with the expected values
     */
    @Test
    public void testListIteratorMethods() {
        HListIterator listIterator = list.hListIterator();
        ...
        listIterator = listWithData.hListIterator();
